...
from lib.models.uds_render import uds_gaussian_render
...
def training():
    ...
    dataset = Dataset()
    gaussians = uds_gaussian_model(dataset.scene_info.metadata)
    scene = Scene(gaussians=gaussians, dataset=dataset)
    netmodel = WhloeModel().to(device="cuda") 
    netoptimizer = torch.optim.Adam(netmodel.parameters(), lr=0.0001)
    ...
    gaussians_renderer = uds_gaussian_render()
    ...
    for iteration in range(start_iter, training_args.iterations + 1):
        ...
        render_pkg = gaussians_renderer.render(viewpoint_cam, gaussians, netmodel)
        ...
        with torch.no_grad():
            ...
            if iteration < training_args.iterations:
                ...
                netoptimizer.step()
                netoptimizer.zero_grad(set_to_none=True)
...
def training_report(tb_writer, iteration, scalar_stats, tensor_stats, testing_iterations, scene: Scene, renderer: uds_gaussian_render):
    if tb_writer:
        try:
