...
from lib.models.uds_gaussian import uds_gaussian_model
from lib.utils.camera_utils import Camera, make_rasterizer
from lib.config import cfg
from lib.models.Netmodel import WhloeModel

class uds_gaussian_render():
    def __init__(
        self,         
    ):
        self.cfg = cfg.render
    ...
    def render_object(
        ...
        result = self.render_kernel(viewpoint_camera, pc, netmodel, convert_SHs_python, compute_cov3D_python, scaling_modifier, override_color, white_background=True)
    ...
    def render_background(
        ...
        pc.parse_camera(viewpoint_camera)
        result = self.render_kernel(viewpoint_camera, pc, WhloeModel, convert_SHs_python, compute_cov3D_python, scaling_modifier, override_color, white_background=True)
    ...
    def render_sky(
        ...
        pc.parse_camera(viewpoint_camera)
        result = self.render_kernel(viewpoint_camera, pc, netmodel,convert_SHs_python, compute_cov3D_python, scaling_modifier, override_color)
        return result
    ...
    def render_kernel(
        ...
        feat = netmodel.forward(pc,viewpoint_camera)
        colors_precomp = torch.sigmoid(feat)   
